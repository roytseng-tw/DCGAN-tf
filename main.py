...
flags.DEFINE_string("dataset_dir", "data/celebA", "Directory contains all the training images")
flags.DEFINE_string("input_fname_pattern", "*.jpg", "Glob pattern of filename of input images")
flags.DEFINE_integer("input_threads", 10, "Number of input threads to read the input in parallel.")

flags.DEFINE_string("run_name", "run1", "Directory name for different runs")
...
flags.DEFINE_integer("ckpt_max_to_keep", 10000, "Max number of checkpoint files to save. `None` means keep all.")
...
def train():
    ...
    try:
        # Write down the FLAG values in FLAGS.txt in `F.checkpoint_dir`
        with open(os.path.join(F.checkpoint_dir, 'FLAGS.txt'), 'w') as fout:
            pprint.pprint(F.__flags, fout)
